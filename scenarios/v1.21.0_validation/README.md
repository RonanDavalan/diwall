...
Basic Auth challenge (RFC 7617) from the vault, scoped to the target's
...
- **T-1** — without `--http-credentials`: `http_status: 401`,
  ...
  appears. Fully portable, no vault required.
- **T-2** — with `--http-credentials` and correct fixture credentials: the
  ...
  requires a mounted gocryptfs vault (skipped otherwise, not failed).
