...
{
  "nom": "my_scenario",
  "intention": "Create new client acme-corp",
  "url": "https://target.local/",
  "actions": [ … ]
}
```
...
```bash
# WRONG — /home/<user>/Vaults/<PROJET>/Diwall/ contains diwall.conf, not hostname.json files
DIWALL_VAULT_DIR=/home/<user>/Vaults/<PROJET>/Diwall/ python3 shot.py …

# CORRECT — points to the .conf file which resolves vault_dir internally
DIWALL_CONF=/home/<user>/Vaults/<PROJET>/Diwall/diwall.conf python3 shot.py …
```
