...
- Audit anomalies inter-session : 5 scénarios non trackés + `__PASSWORD_FACTICE__` en clair dans scénarios déjà commités depuis session 31.
...
- `scripts/preflight-publication.sh` : périmètre étendu aux `scenarios/*.json`, pattern `__PASSWORD_FACTICE__` ajouté.
...
**Décision technique :** pas de réécriture d'historique git pour `__PASSWORD_FACTICE__` (commit explicatif transparent préféré — password factice, app dev local sans authentification requise, nettoyage par principe).
